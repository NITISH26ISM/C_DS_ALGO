...
void dijkstras(list<edge> adj[], int v ){
    binaryMinHeap Q(v);
    vector<int> parent;
    parent.resize(v);
    Q.insert(node(0, 0));
    ...
    vector<int> shortest_dis( v, INT_MAX);
    
    while( !Q.isEmpty() ){
        node n_u = Q.extract_min();
        int u = n_u.vertice;
        shortest_dis[u] = n_u.priority;
        
        for( auto itr = adj[u].begin(); itr != adj[u].end(); itr++ ){
            int v = (*itr).v;
            int e = (*itr).wt;
            if( Q.contain(v) ){
                if( Q.key(v) > shortest_dis[u] + e ){
                    Q.decrease_key(v, shortest_dis[u] + e );
                    parent[v] = u;
                }
            }
        }
    ...
    for( int i = 0; i < v; i++ ){
        cout<<" vertice "<<i<<" path distance "<<shortest_dis[i]<<endl;
        int j = i;
        while( j != 0 ){
            cout<<j<<"<---";
            j = parent[j];
